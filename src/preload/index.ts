...
const api = {
  ...
  reconcileTagFolders: (opts?: {
    onlyCivitaiTags?: string[]
  }): Promise<{
    moved: number
    skipped: number
    queueUpdated: number
    versionIds: number[]
  }> => ipcRenderer.invoke('inventory:reconcileTagFolders', opts),
  ...
  setDownloadRouting: (versionId: number, routingTag: string): Promise<DownloadQueueState> =>
    ipcRenderer.invoke('download:setRouting', { versionId, routingTag }),
